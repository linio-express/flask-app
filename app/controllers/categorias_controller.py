...
categoria = Blueprint('categoria', 'api', template_folder=template_dir)
...
def show(id_categoria):
    ...
    if session.get('logged_in') == True:
        ...
        return render_template('productos/index.html', resultados = toJSON(productos), texto_a_buscar=texto_a_buscar, usuario=usuario.toJSON(), categoria = categoria.toJSON())
    else:
        session["current_user_id"] = 0
        session["logged_in"] = False
        return render_template('productos/index.html', logged_in = session["logged_in"], current_user_id = session["current_user_id"], resultados = toJSON(lista_productos), texto_a_buscar=texto_a_buscar, categoria = categoria.toJSON())
